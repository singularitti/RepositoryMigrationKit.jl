...
!!! info
    The style guide is always a work in progress, and not all RepositoryMigrationKit code
    follows the rules. When modifying RepositoryMigrationKit, please fix the style violations
...
RepositoryMigrationKit uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/RepositoryMigrationKit.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the RepositoryMigrationKit directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to RepositoryMigrationKit have
