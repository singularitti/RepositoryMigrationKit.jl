...
[RepositoryMigrationKit](https://github.com/singularitti/RepositoryMigrationKit.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("RepositoryMigrationKit")
   ```

3. Run

   ```julia-repl
   julia> using RepositoryMigrationKit
   ```
...
version of RepositoryMigrationKit, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/singularitti/RepositoryMigrationKit.jl"
```
...
our [GitHub repository](https://github.com/singularitti/RepositoryMigrationKit.jl)
for new releases.
Once we release a new version, you can update RepositoryMigrationKit by typing

```@repl
using Pkg
Pkg.update("RepositoryMigrationKit")
Pkg.gc()
```
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("RepositoryMigrationKit")

   julia> Pkg.gc()
   ```

2. Press `Ctrl+D` to quit the current session. Start a new Julia session and
   reinstall RepositoryMigrationKit.
